...
- **transition_state_publish_interval** (*Optional*, [Time](/guides/configuration-types#time)): How often to publish
  the light's state while a transition or flash is running, so Home Assistant and other clients can follow a long
  fade. The starting state is published when the call is made, intermediate states at this interval, and the final
  state once at the end. Calls made during the fade still start from the fade's target, and `save: true` saves the
  target. Not available on addressable lights. Values below about `150ms` do not publish any faster on an ESP8266 and
  only add CPU load and API/MQTT traffic; `200ms` to `250ms` works well. Defaults to `0s`, which keeps the existing
  behaviour of publishing the target once when the call is made.
...
- **on_state** (*Optional*, [Action](/automations/actions#all-actions)): An automation to perform when the light's set state is
  changed. See [`light.on_state` Trigger](#light-on_state-trigger).
...
- **power_supply** (*Optional*, [ID](/guides/configuration-types#id)): The [Power Supply](/components/power_supply/) to connect to this light. When
...
<span id="light-is_on_condition"></span>
<span id="light-is_off_condition"></span>
...
<span id="light-on_turn_on_off_trigger"></span>
...
| [E1.31](#e1-31-effect) | Receive sACN / E1.31 frames over the network | `universe`, `channels` |
...
<span id="light-effect-get_name"></span>
...
- **it** - <APIClass text="AddressableLight" path="light::AddressableLight" /> instance (see API reference for more info).
- **current_color** - <APIStruct text="ESPColor" path="light::ESPColor" /> instance (see API reference for more info).
- **initial_run** - A bool which is true on the first execution of the lambda. Useful to reset static variables when
  restarting an effect.

> [!NOTE]
> ESPColor has been migrated to Color. See <APIStruct text="Color" path="Color" /> for more information.
...
- <APIRef text="light_state.h" path="light/light_state.h" />
