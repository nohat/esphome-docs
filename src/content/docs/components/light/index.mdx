...
- **transition_state_publish_interval** (*Optional*, [Time](/guides/configuration-types#time)): How often to publish
  ...
  target. Not available on addressable lights. Must be at least `150ms`; `200ms` to `250ms` works well. When not
  set, the light publishes the target once when the call is made, as before.
