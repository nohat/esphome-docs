...
- **transition_state_publish_interval** (*Optional*, [Time](/guides/configuration-types#time)): How often to publish
  ...
  state once at the end; the `on_state` trigger fires for each of them. Calls made during the fade still start from
  the fade's target, and `save: true` saves the target. Not available on addressable lights. Minimum is `150ms`; `200ms` to `250ms` works well. When not set,
  the light publishes the target once when the call is made, as before.
