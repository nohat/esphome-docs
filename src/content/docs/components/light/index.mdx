...
- **transition_state_publish_interval** (*Optional*, [Time](/guides/configuration-types#time)): How often to publish
  ...
  the fade's target, and `save: true` saves the target. Not available on addressable lights. Minimum is `150ms`;
  `200ms` to `250ms` works well. When not set, only the target state is published, once, when the call is made.
